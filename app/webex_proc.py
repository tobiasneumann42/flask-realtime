        ...
        for userid in jResponse['hits']['hits']:
            ...
            if 'userId' in userid['_source']:
              ...
              if userid['_source']['userId']:
                log.debug(f"Checking userinfo already in elastic for user {userid['_source']['userId']}")
                uinfo = get_user_info_elastic( userid['_source']['userId'] )
                if not uinfo:
                  log.debug(f"No entry in elastic: {userid['_source']['userId']}")
                  uinfo = get_user_info_webex( userid['_source']['userId'], access_token )
                  print(f"Fetched entry from webex cloud: %s ", uinfo )
                # execute update
                if uinfo:
                  print(f"Execute update userinfo in elastic: {userid['_index']} - {userid['_id']}")
                  update_userinfo( userid, uinfo )
                  records_updated  += 1
                  count += 1
        
        print(f"### DEBUG: records updated: {records_updated}")
